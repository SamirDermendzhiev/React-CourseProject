...
export function RecipeCard({ recipe, onDelete }) {
  ...
  return (
    <div className="note-card w-100 p-3">
      <div className="card">
        <img className="card-img-top ratio-1 cover" src={recipe.picture} alt={recipe.name} />
        ...
        {(loggedUser.id === recipe.userId || loggedUser.isAdmin) && (
          <div>
            <Link to={`/recipes/edit/${recipe.id}`} className="m-0">
              Edit{" "}
            </Link>
            <span
              className="card-link"
